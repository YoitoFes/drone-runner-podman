...
import (
	...
	"github.com/YoitoFes/drone-runner-podman/engine"
	"github.com/YoitoFes/drone-runner-podman/engine/compiler"
	"github.com/YoitoFes/drone-runner-podman/engine/linter"
	"github.com/YoitoFes/drone-runner-podman/engine/resource"
	"github.com/YoitoFes/drone-runner-podman/internal/match"
...
func (c *daemonCommand) run(*kingpin.ParseContext) error {
	...
	for {
		...
		if err != nil {
			logrus.WithError(err).
				Errorln("cannot ping podman")
			time.Sleep(time.Second)
		} else {
			logrus.Debugln("successfully pinged podman")
			break
		}
	}
